...
const INTERVIEW_SYSTEM_PROMPT = `Você é a Corujinha conduzindo uma entrevista breve, calorosa e investigativa para entender por que esta pessoa específica tem dificuldade em deitar no horário que ela mesma escolheu.
...
export async function summarizeInterview(
  ...
  const transcript = history
    .map((m) => `${m.role === 'corujinha' ? 'Corujinha' : 'Pessoa'}: ${m.content}`)
    .join('\n');
